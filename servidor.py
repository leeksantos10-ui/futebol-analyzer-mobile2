...
def read_rows(filename, data):
    ...
    if ext=="xlsx":
        ...
        for vals in it:
            ...
            rows.append({headers[i]: vals[i] if i<len(vals) else None for i in range(len(headers)) if headers[i]})
        return headers, rows
    if ext=="csv":
...
def import_rows(rows, update_existing=False):
    con=sqlite3.connect(DB); cur=con.cursor(); dbcols=[r[1] for r in cur.execute("PRAGMA table_info(jogos)").fetchall()]; allowed=set(dbcols)
    ...
    for raw in rows:
        ...
        if any(row.get(k) in (None,"") for k in required): invalid+=1; continue
        key=(str(row.get("Data","")).strip(),str(row.get("Hora","")).strip(),str(row.get("Confronto","")).strip(),str(row.get("Campeonato","")).strip())
        ex=cur.execute('SELECT rowid FROM jogos WHERE TRIM(CAST("Data" AS TEXT))=? AND TRIM(CAST("Hora" AS TEXT))=? AND TRIM(CAST("Confronto" AS TEXT))=? AND TRIM(CAST("Campeonato" AS TEXT))=?',key).fetchone()
        if ex:
            if not update_existing: dupes+=1; continue
            sets=[c for c in row if c not in {"Data","Hora","Confronto","Campeonato"}]
            if sets:
                cur.execute('UPDATE jogos SET '+','.join('"'+c.replace('"','""')+'"=?' for c in sets)+' WHERE rowid=?',[row[c] for c in sets]+[ex[0]])
                updated+=1
            else: dupes+=1
        else:
            cols=list(row); cur.execute('INSERT INTO jogos ('+','.join('"'+c.replace('"','""')+'"' for c in cols)+') VALUES ('+','.join('?' for _ in cols)+')',[row[c] for c in cols]); inserted+=1
    con.commit(); con.close(); return {"inserted":inserted,"updated":updated,"duplicates":dupes,"invalid":invalid,"total_processed":len(rows)}
        ...
        try:
            ...
            if u.path in ("/api/import/preview","/api/import"):
                ...
                if u.path=="/api/import/preview":
                    ...
                    return self.json({"status":"ok","filename":filename,"rows":len(rows),"headers":headers,"matched":matched,"preview":rows[:8]})
